...
async def audio_playing(ctx):
    ...
    if client and client.channel and client.source:
        ...
        raise commands.CommandError("В настоящее время не воспроизводится песня.")
...
async def in_voice_channel(ctx):
    ...
    if voice and bot_voice and voice.channel and bot_voice.channel and voice.channel == bot_voice.channel:
        ...
        raise commands.CommandError(
            "Вы должны быть в канале, чтобы сделать это")
...
class Music(commands.Cog):
    """Команды бота, чтобы помочь воспроизвести музыку."""
    ...
    async def leave(self, ctx):
        """Покидает голосовой канал, если он в данный момент."""
        client = ctx.guild.voice_client
    ...
    async def pause(self, ctx):
        """Приостанавливает любое воспроизводимое в данный момент аудио."""
        client = ctx.guild.voice_client
    ...
    async def volume(self, ctx, volume: int):
        """Изменить громкость звука (значения 0-250)."""
        state = self.get_state(ctx.guild)
    ...
    async def skip(self, ctx):
        """Пропускает текущую песню или голосование."""
        state = self.get_state(ctx.guild)
        client = ctx.guild.voice_client
    ...
    def _queue_text(self, queue):
        ...
        if len(queue) > 0:
            ...
            message += [
                f"  {index+1}. **{song.title}** (запрошенный **{song.requested_by.name}**)"
    ...
    async def clearqueue(self, ctx):
        """Очищает очередь воспроизведения, не выходя из канала."""
        state = self.get_state(ctx.guild)
        state.playlist = []
    ...
    #     """Перемещает песню по индексу в `new_index` в очереди."""
    ...
    @commands.command(brief="Воспроизведение аудио с ссылки или с названия.")
    @commands.guild_only()
    async def play(self, ctx, *, url):
        """Воспроизводит аудио, размещенное по адресу <url> (или выполняет поиск <url> и воспроизводит первый результат)."""
        ...
        if client and client.channel:
            ...
            if ctx.author.voice is not None and ctx.author.voice.channel is not None:
                ...
                raise commands.CommandError(
                    "Для этого вам нужно быть в голосовом канале.")
